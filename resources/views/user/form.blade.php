...
<div class="form-group {{ $errors->has('rememberToken') ? 'has-error' : ''}}">
    <label for="rememberToken" class="control-label">{{ 'Remembertoken' }}</label>
    <input class="form-control" name="rememberToken" type="text" id="rememberToken" value="{{ isset($user->rememberToken) ? $user->rememberToken : ''}}" >
    {!! $errors->first('rememberToken', '<p class="help-block">:message</p>') !!}
</div>
